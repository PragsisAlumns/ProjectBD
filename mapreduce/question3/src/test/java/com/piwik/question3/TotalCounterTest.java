...
import com.piwik.totalcounter.TotalCounterPageReducer;
...
public class TotalCounterTest {
	...
	public void setUp() {
		...
		TotalCounterPageMapper totalCounterPageMapper = new TotalCounterPageMapper();

		mapDriver = new MapDriver<LongWritable, Text, Text, LongWritable>();
		mapDriver.setMapper(totalCounterPageMapper);

		// Set up the reducer test harness.
		TotalCounterPageReducer totalCounterPageReducer = new TotalCounterPageReducer();
		reduceDriver = new ReduceDriver<Text, LongWritable, Text, LongWritable>();
		reduceDriver.setReducer(totalCounterPageReducer);

		// Set up the mapper/reducer test harness.
		ConvertPageCombiner totalCounterPageCombiner = new ConvertPageCombiner();
		mapReduceDriver = new MapReduceDriver<LongWritable, Text, Text, LongWritable, Text, LongWritable>();
		mapReduceDriver.setMapper(totalCounterPageMapper);
		mapReduceDriver.setReducer(totalCounterPageReducer);
		mapReduceDriver.setCombiner(totalCounterPageCombiner);
	}
	...
	public void testMapper() throws IOException {
		...
		Text key4 = new Text("5");
		Text key5 = new Text("2");
		Text key6 = new Text("4");
		Text key7 = new Text("3");
		Text key8 = new Text("4");
	...
	public void testMapReduce() throws IOException {
		...
		//Given
		String lineHDFS1 = "23	5,4,5#3,4,3,4";
		...
		.withOutput(key1, new LongWritable(2))
		.withOutput(key2, new LongWritable(4))
